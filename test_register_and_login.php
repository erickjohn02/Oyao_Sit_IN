...
if ($stmt->execute()) {
    ...
    // Now test login
    $login_query = "SELECT 'user' AS role, username, password FROM users WHERE username = ?";
    ...
    if ($result->num_rows > 0) {
        ...
        if (password_verify($test_user['password'], $user['password'])) {
            echo "<br>Login test successful!<br>";
            echo "You can now login with:<br>";
            echo "Username: " . $test_user['username'] . "<br>";
            echo "Password: " . $test_user['password'] . "<br>";
        } else {
            echo "<br>Login test failed!<br>";
            echo "This means there's an issue with the password verification.<br>";
        }
    } else {
        echo "Error: User not found after registration!<br>";
    }
} else {
