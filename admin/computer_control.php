...
if (isset($_POST['update_status'])) {
    ...
    foreach ($pcs as $pc) {
        $pc_num = intval($pc);
        $conn->query("INSERT INTO lab_pcs (lab, pc_number, status) VALUES ('{$lab}', {$pc_num}, '{$status}') ON DUPLICATE KEY UPDATE status='{$status}'");
    }
    echo "<script>alert('PC status updated!'); window.location.href='computer_control.php?lab={$lab}';</script>";
}
...
<div class="container mt-4">
    ...
    <form method="GET" class="mb-3 d-flex gap-2 align-items-center">
        ...
        <a href="computer_control.php?lab=<?= $selected_lab ?>" class="btn btn-secondary btn-sm">Show All</a>
    </form>
    <form method="POST">
